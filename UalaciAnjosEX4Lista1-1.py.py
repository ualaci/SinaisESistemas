...
#funcao que plota o sinal das funcoes discretas
...
def main():
    #foram criados varios vetores de tempo para adequar a funcao original aos intervalos nos quais elas são deslocadas
    t = np.arange(-10, 20, 1)
    t3 = np.arange(-10, -6, 1)
    t5 = np.arange(-14, 16, 1)
    y2 = []
    #t = np.linspace(-10,10,1000)
    #como x1 e x2 do enunciado do exercicio possuem o mesmo valor
    #no intervalo de -10 a 20, nao havera diferenca na analise do intervalo de -10 a 20 incialmente
    #As chamas de plota sinal comentadas representam as funcoes que foram somadas pra adquirir o resultado do exercicio
    tam = len(t)
    x0 = 0*t3
    x1 = np.cos((pi/6)*t)
    x2 = np.cos((pi/6)*t)
